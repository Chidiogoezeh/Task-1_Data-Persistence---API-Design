...
export const fetchGenderData = async (name) => {
  ...
  if (!res.data.gender || res.data.count === 0) throw createExternalError('Genderize');
  return {
...
export const fetchAgeData = async (name) => {
  ...
  if (res.data.age <= 12) age_group = 'child';
  else if (res.data.age <= 19) age_group = 'teenager';
  else if (res.data.age <= 59) age_group = 'adult';
...
export const fetchNationalData = async (name) => {
  ...
  if (!res.data.country || res.data.country.length === 0) throw createExternalError('Nationalize');
